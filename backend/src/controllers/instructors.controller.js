...
async function getAllInstructors(req, res) {
  try {
    const result = await pool.query(
      `SELECT id, full_name, email, department_id, created_at, updated_at
       FROM instructors
       WHERE owner_id = $1
...
async function createInstructor(req, res) {
  try {
    ...
    const result = await pool.query(
      `INSERT INTO instructors (full_name, email, department_id, owner_id)
       VALUES ($1, $2, $3, $4)
       RETURNING id, full_name, email, department_id, created_at, updated_at`,
      [String(fullName).trim(), email ? String(email).trim() : null, Number(departmentId), req.user.id],
    );
  ...
  try {
    ...
    const result = await pool.query(
      `UPDATE instructors
       ...
       RETURNING id, full_name, email, department_id, created_at, updated_at`,
      [String(fullName).trim(), email ? String(email).trim() : null, Number(departmentId), id, req.user.id],
    );
