...
async function createRoom(req, res) {
  try {
    ...
    const result = await pool.query(
      ...
      [String(roomCode).trim(), String(building).trim(), parsedCapacity, Boolean(isActive), req.user.id],
    );

    return res.status(201).json({ success: true, message: "Room created successfully", data: result.rows[0] });
  } catch (error) {
...
async function updateRoom(req, res) {
  try {
    ...
    const result = await pool.query(
      `UPDATE rooms
       SET room_code = $1, building = $2, capacity = $3, is_active = $4, updated_at = NOW()
       WHERE id = $5 AND owner_id = $6
       RETURNING id, room_code, building, capacity, is_active`,
      [String(roomCode).trim(), String(building).trim(), parsedCapacity, Boolean(isActive), id, req.user.id],
    );
...
async function toggleRoomActive(req, res) {
  try {
    ...
    return res.status(200).json({ success: true, message: "Room active status updated successfully", data: result.rows[0] });
  } catch (error) {
    console.error("Toggle room active error:", error);
    return res.status(500).json({ success: false, message: "Failed to toggle room status", error: error.message });
  }
}
...
async function deleteRoom(req, res) {
  try {
    const { id } = req.params;
    const result = await pool.query(
      `DELETE FROM rooms WHERE id = $1 AND owner_id = $2 RETURNING id, room_code`,
      [id, req.user.id],
    );
    if (result.rows.length === 0) {
      return res.status(404).json({ success: false, message: "Room not found" });
