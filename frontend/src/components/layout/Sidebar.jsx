...
function SectionButton({ icon: Icon, label, description, isActive, isOpen, onClick }) {
  ...
  return (
    ...
    >
      ...
      {!isActive && (
        <motion.div className="absolute inset-0 rounded-lg opacity-0 bg-accent/60 transition-opacity hover:opacity-100" />
      )}
      <motion.span
        variants={shouldReduce ? {} : iconVariants}
        transition={{ type: "spring", stiffness: 400, damping: 20 }}
...
export default function Sidebar({ mobileOpen = false, onMobileClose }) {
  ...
  const sidebarInner = (
    <>
      {/* ── Logo ── */}
      <div className="flex h-14 items-center border-b border-sidebar-border px-5 shrink-0">
      ...
      <nav className="flex-1 space-y-1 overflow-y-auto p-3">
        ...
        <motion.button
          type="button"
          onClick={() => { setScheduleModalOpen(true); onMobileClose?.(); }}
