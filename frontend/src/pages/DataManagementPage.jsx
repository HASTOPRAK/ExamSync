          ...
          {onSearch && (
            <div className="relative">
              ...
              <input
                ...
                className="h-8 w-32 rounded-lg border border-border bg-background pl-8 pr-3
    ...
    <tr key={i} className="border-b border-border/40">
      {Array.from({ length: cols }, (_, j) => (
        <td key={j} className="px-4 py-3">
          <div
            className={`h-3.5 animate-pulse rounded bg-muted ${j === 0 ? "w-20" : j === cols - 1 ? "w-12" : "w-32"}`}
          />
        </td>
      ))}
    </tr>
...
function LeftNav({ active, onSelect, counts }) {
  return (
    <nav className="hidden lg:flex w-52 shrink-0 flex-col border-r border-border p-3">
      {/* Data group */}
...
function TabStrip({ active, onSelect, counts }) {
  return (
    <div className="flex lg:hidden shrink-0 overflow-x-auto border-b border-border px-2 py-2 gap-1">
...
export default function DataManagementPage() {
  ...
  async function handleRemoveAssignment(instructorId) {
    if (!selectedCourseId) return;
    const ok = await confirm({
      title: "Remove this instructor from the course?",
      destructive: true,
      confirmLabel: "Remove",
    });
    if (!ok) return;
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        className="flex h-[60svh] flex-col overflow-hidden rounded-2xl border border-border bg-card/40 lg:flex-row lg:h-[calc(100svh-23rem)]"
      >
        {/* Mobile tab strip */}
        <TabStrip
                        ...
                        <EmptyRow
                          ...
                          message={
                            roomSearch
                              ...
                              : "No rooms yet. Hit Add Room to create one."
                          }
                        />
                      ) : (
                        ...
                        <EmptyRow
                          ...
                          message={
                            instructorSearch
                              ...
                              : "No instructors yet. Hit Add Instructor to create one."
                          }
                        />
                      ) : (
                            ...
                            <Td>
                              <div className="flex items-center gap-1.5">
                                <button
                                  type="button"
                                  onClick={() =>
                                    openInstructorSheet(instructor)
                                  }
                                  className="rounded-md px-2.5 py-1 text-xs font-medium text-muted-foreground border border-border hover:bg-accent hover:text-foreground transition-colors"
                                >
                                  Edit
            ...
            {activeSection === "assignments" && (
              ...
              >
                <div className="flex h-full flex-col sm:flex-row">
                  {/* Mobile: course select dropdown */}
                  <div className="sm:hidden shrink-0 border-b border-border px-4 py-3">
                    <select
                      value={selectedCourseId}
                      onChange={(e) => setSelectedCourseId(e.target.value)}
                      className="w-full h-9 rounded-md border border-border bg-background px-3 text-sm text-foreground outline-none focus:border-primary"
                    >
                      {courses.length === 0 ? (
                  ...
                  {/* Desktop: course list sidebar */}
                  <div className="hidden sm:flex w-56 shrink-0 flex-col overflow-y-auto border-r border-border">
                  ...
                  {selectedCourse ? (
                    <div className="flex min-w-0 flex-1 flex-col divide-y divide-border overflow-hidden sm:flex-row sm:divide-x sm:divide-y-0">
                      {/* Assigned */}
                            ...
                            {assignments.map((a) => (
                              ...
                              >
                                ...
                                <button
                                  type="button"
                                  onClick={() =>
                                    handleRemoveAssignment(a.instructor_id)
                                  }
                                  title="Remove"
      ...
      <motion.div
        variants={shouldReduce ? {} : fadeUp}
        className="flex flex-wrap items-center justify-between gap-4 rounded-xl border border-destructive/20 bg-destructive/5 px-5 py-4"
      >
        <div>
          <p className="text-sm font-semibold text-destructive">Danger Zone</p>
          <p className="mt-0.5 text-xs text-muted-foreground">
            Permanently removes all rooms, instructors, courses, students, and
            enrollments. Exam periods and schedules are unaffected.
          </p>
        </div>
        <Button
          variant="destructive"
